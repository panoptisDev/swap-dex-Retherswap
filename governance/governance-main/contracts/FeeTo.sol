...
contract FeeTo {
    ...
    function renounce(address pair) public returns (uint value) {
        ...
        TokenAllowState storage token0AllowState = tokenAllowStates[IRetherswapPair(pair).token0()];
        TokenAllowState storage token1AllowState = tokenAllowStates[IRetherswapPair(pair).token1()];
        ...
        if (
            ...
            value = IRetherswapPair(pair).balanceOf(address(this));
            if (value > 0) {
                ...
                assert(IRetherswapPair(pair).transfer(pair, value));
                IRetherswapPair(pair).burn(pair);
            }
    ...
    function claim(address pair) public returns (uint value) {
        ...
        if (
            ...
            if (value > 0) {
                ...
                assert(IRetherswapPair(pair).transfer(feeRecipient, value));
            }
        }
    }
}

interface IRetherswapPair {
